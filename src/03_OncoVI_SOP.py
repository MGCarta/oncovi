...
Created on Sat Jun 31 09:27:46 2025
    ...
    output of the VEP-based pipeline.    
...
if __name__ == '__main__':
    ...
    # An ArgumentParser object is created to analyze multiple given 
    # strings from command-line by the user 
    
    parser = argparse.ArgumentParser(prog='OncoVI', 
                                     description="Evaluates oncogenicity guidelines for the VEP output")
    
    # Parser arguments    
    
    # Input vcf file folder
    # This argument is mandatory
    parser.add_argument(
        '-i','--input',
        type = str,
        required = True,
        help = 'Path to the folder containing the VEP output'
        )   
    ...
    parser.add_argument(
        ...
        type = str,
        required = True,
        help = 'Path to the resources needed by OncoVI'
        )
    ...
    parser.add_argument(
        ...
        type = str,
        required = False,
        help = 'Name of the sample under evaluation'
        )
...
print("Your input data to OncoVI evaluation: ")
print(f"{input_data}")
print("")
...
## Path for the OncoVI output 
output_dir = os.path.join(os.path.dirname(input_data), str(sample_name)+"_OncoVI")

## Create an output directory unless it doesn´t exist
if os.path.isdir(output_dir) == True:
    pass
else:
    os.mkdir(output_dir)
    
print("Creating folder for OncoVI evaluation output: ")
print(f"{output_dir}")
print("")
...
print(f"{Fore.CYAN}Upload resources required by OncoVI")
...
print(f"{Fore.CYAN}Name of input data processed {sample_name}")
print(f"{Fore.CYAN}-" * 100)
print("")
        
# Read in data    
data_init = pd.read_excel(input_data, na_filter=False)
...
print(f"{Fore.CYAN}Run OncoVI on the data: {sample_name}")
print(f"{Fore.CYAN}-" * 100)
print("")
...
for index, row in data_init.iterrows():
    ...
    # Change the variant identifier to get rid of the duplicates
    new_identifier = str(chrom) + "_" + str(pos) + "_" + ref_allele + "_" + alt_allele
    ...
    if gene not in tsg_bona_fide_list:
        ...
        criteria_df.loc[new_identifier, "OVS1"] = "no"
        points_df.loc[new_identifier, "OVS1_p"] = 0
    ...
    # The gene is in TSGs list    
        ...
        if any(cons in consequence for cons in null_variants):
          ...
          criteria_df.loc[new_identifier, "OVS1"] = 'yes'
          points_df.loc[new_identifier, "OVS1_p"] = 8 
    ...
    if (criteria_df.loc[new_identifier, 'OVS1'] == "no") and gene in mut_splice_dict.keys():
    ...
    if hgvsp != "":
        ...
        if var_id not in onco_var_cgi.keys():
                ...
                criteria_df.loc[new_identifier, 'OS1'] = 'no'
                points_df.loc[new_identifier, 'OS1_p'] = 0
                
        else:
            
            criteria_df.loc[new_identifier, 'OS1'] = 'yes'
            points_df.loc[new_identifier, 'OS1_p'] = 4
    ...
    if ("Pathogenic" in clinvar_classification) and (review_status in review_level_list):
        ...
        criteria_df.loc[new_identifier, 'OS2'] = 'yes'
        points_df.loc[new_identifier, 'OS2_p'] = 4
        ...
        criteria_df.loc[new_identifier, 'OS2'] = 'no'
        points_df.loc[new_identifier, 'OS2_p'] = 0
    ...
    if criteria_df.loc[new_identifier, 'OS1'] == "yes":
        
        # This avoid verifying the presence of the variant in COSMIC
        res_cancerhotspots = "True"
        
        criteria_df.loc[new_identifier, 'OS3'] = "no"
        points_df.loc[new_identifier, 'OS3_p'] = 0
                        ...
                        if (amino_acid_position != "") and (amino_acid_position == key):
                            ...
                            if len(res_variant_inframe_indel) == 0:
                                ...
                                if (sum(list(map(int, gene_records_inframe_indel_dict[key].values()))) >= 50) and (int(res_variant_inframe_indel[0]) >= 10):
                                    ...
                                    criteria_df.loc[new_identifier, 'OS3'] = 'yes'
                                    points_df.loc[new_identifier, 'OS3_p'] = 4
                                    break;
    ...
    if (res_cancerhotspots == "False") and (criteria_df.loc[new_identifier, 'OS3'] == "no") and (criteria_df.loc[new_identifier, 'OS1'] == "no"):
    ...
    if (criteria_df.loc[new_identifier, 'OS1'] == "yes") or (criteria_df.loc[new_identifier, 'OS3'] == "yes"):
        
        criteria_df.loc[new_identifier, 'OM1'] = "no"
        points_df.loc[new_identifier, 'OM1_p'] = 0
            ...
            if "-" not in amino_acid_position: 
                ...
                for domain in domains_gene:
                    ...
                    if int(start_domain) <= int(amino_acid_position) <= int(end_domain):
                        ...
                        criteria_df.loc[new_identifier, 'OM1'] = "yes"
                        points_df.loc[new_identifier, 'OM1_p'] = 2
                        break;
    ...
    if criteria_df.loc[new_identifier, 'OVS1'] == "yes":
        
        criteria_df.loc[new_identifier, 'OM2'] = "no"
        points_df.loc[new_identifier, 'OM2_p'] = 0
        
    else:
        
        if any(cons in consequence for cons in indels) and ((gene in oncogenes_list) or (gene in tsg_list)):
                    
            criteria_df.loc[new_identifier, 'OM2'] = "yes"
            points_df.loc[new_identifier, 'OM2_p'] = 2
    ...
    if ((criteria_df.loc[new_identifier, 'OS3'] == "yes") or (criteria_df.loc[new_identifier, 'OM1'] == "yes")):
        
        criteria_df.loc[new_identifier, 'OM3'] = "no"
        points_df.loc[new_identifier, 'OM3_p'] = 0
                ...
                for item in res:
                    ...
                    try:
                        ...
                        if new_grantham_score >= original_grantham_score:
                            ...
                            criteria_df.loc[new_identifier, 'OM3'] = "yes"
                            points_df.loc[new_identifier, 'OM3_p'] = 2
                            break;
    ...
    if criteria_df.loc[new_identifier, 'OS3'] == "yes":
        
        criteria_df.loc[new_identifier, 'OM4'] = "no"
        points_df.loc[new_identifier, 'OM4_p'] = 0
        
    elif (criteria_df.loc[new_identifier, 'OM1'] == "yes") or (criteria_df.loc[new_identifier, 'OM3'] == "yes"):
                        ...
                        if (amino_acid_position != "") and (amino_acid_position == key):
                            ...
                            if len(res_variant_inframe_indel) == 0:
                                ...
                                if (sum(list(map(int, gene_records_inframe_indel_dict[key].values()))) < 50) and (int(res_variant_inframe_indel[0]) >= 10):
                                    ...
                                    criteria_df.loc[new_identifier, 'OM4'] = 'yes'
                                    points_df.loc[new_identifier, 'OM4_p'] = 2
                                    break;
    ...
    if (spliceAI_score == "") and (phylop_score == "") and (phastCons_score == ""):
        ...
        criteria_df.loc[new_identifier, 'OP1'] = 'no'
        points_df.loc[new_identifier, 'OP1_p'] = 0
            ...
            criteria_df.loc[new_identifier, 'OP1'] = 'yes'
            points_df.loc[new_identifier, 'OP1_p'] = 1
    ...
    if criteria_df.loc[new_identifier, 'OS3'] == "yes": 
        ...
        criteria_df.loc[new_identifier, 'OP3'] = 'no'
        points_df.loc[new_identifier, 'OP3_p'] = 0
        
    elif criteria_df.loc[new_identifier, 'OM4'] == "yes":
                        ...
                        if (amino_acid_position != "") and (amino_acid_position == key):
                            ...
                            if len(res_variant_inframe_indel) == 0:
                                ...
                                if int(res_variant_inframe_indel[0]) < 10:
                                    ...
                                    criteria_df.loc[new_identifier, 'OP3'] = 'yes'
                                    points_df.loc[new_identifier, 'OP3_p'] = 1
                                    break;
    ...
    if (gnomADe_freq == "") and (gnomADg_freq == ""): 
        ...
        criteria_df.loc[new_identifier, 'OP4'] = 'yes'
        points_df.loc[new_identifier, 'OP4_p'] = 1
        ...
        criteria_df.loc[new_identifier, 'OP4'] = 'no'
        points_df.loc[new_identifier, 'OP4_p'] = 0
    ...
    if (real_gnomADg_freq_pop == []) and (real_gnomADe_freq_pop == []):
        ...
        criteria_df.loc[new_identifier, 'SBVS1'] = 'no'
        points_df.loc[new_identifier, 'SBVS1_p'] = 0
        
    elif any(float(item) > 0.05 for item in real_gnomADg_freq_pop):   
         
        criteria_df.loc[new_identifier, 'SBVS1'] = 'yes'
        points_df.loc[new_identifier, 'SBVS1_p'] = -8
    ...
    if (real_gnomADg_freq_pop == []) and (real_gnomADe_freq_pop == []):
        ...
        criteria_df.loc[new_identifier, 'SBS1'] = 'no'
        points_df.loc[new_identifier, 'SBS1_p'] = 0
        
    elif any(float(item) > 0.01 for item in real_gnomADg_freq_pop):   
         
        criteria_df.loc[new_identifier, 'SBS1'] = 'yes'
        points_df.loc[new_identifier, 'SBS1_p'] = -4
    ...
    if ("Benign" in clinvar_classification) and (review_status in review_level_list):
        ...
        criteria_df.loc[new_identifier, 'SBS2'] = 'yes'
        points_df.loc[new_identifier, 'SBS2_p'] = -4
        ...
        criteria_df.loc[new_identifier, 'SBS2'] = 'no'
        points_df.loc[new_identifier, 'SBS2_p'] = 0
    ...
    if (spliceAI_score == "") and (phylop_score == "") and (phastCons_score == ""):
        ...
        criteria_df.loc[new_identifier, 'SBP1'] = 'no'
        points_df.loc[new_identifier, 'SBP1_p'] = 0
        ...
        criteria_df.loc[new_identifier, 'SBP1'] = 'yes'
        points_df.loc[new_identifier, 'SBP1_p'] = -1
    ...
    if "synonymous_variant" in consequence:
        ...
        if (spliceAI_score == "") and (phylop_score == "") and (phastCons_score == ""):
            ...
            criteria_df.loc[new_identifier, 'SBP2'] = 'no'
            points_df.loc[new_identifier, 'SBP2_p'] = 0
        
        elif (spliceAI_score != "" and spliceAI_score == "FAIL") and (phylop_score != "" and phylop_score < 0.5) and (phastCons_score != "" and phastCons_score < 0.5):
            
           criteria_df.loc[new_identifier, 'SBP2'] = 'yes'
           points_df.loc[new_identifier, 'SBP2_p'] = -1
    ...
    # Sum of the points
    sum_points = points_df.loc[new_identifier].sum()
    ...
    if new_identifier not in classification_dict.keys():
        
        classification_dict[new_identifier] = {}
        classification_dict[new_identifier] = {'Points' : sum_points,
    ...
    triggered_criteria_list = criteria_df.loc[new_identifier][criteria_df.loc[new_identifier] == "yes"].index.astype(str).tolist()
    
    # triggered criteria stored in a data frame
    triggered_criteria_df.loc[new_identifier, 'Criteria'] = ','.join(triggered_criteria_list)
    
# -------------------------------------------------------------------------- ##

### SAVING THE OncoVI OUTPUT

print(f"{Fore.CYAN}Saving the final OncoVI output for {sample_name}")
print(f"{Fore.CYAN}-" * 100)
print("")

# Data frame with the classification (quantitative and qualitative) of the assessed variants 
...
# Set "new_identifier" column
classification_df_final['new_identifier'] = classification_df_final.index

# Save to file
output_data_dir = os.path.join(output_dir, str(sample_name) + "_OncoVI_eval.csv")
classification_df_final.to_csv(output_data_dir, sep = ',', index = True)

# List of columns to remove
columns_to_remove = ['HGVSg', 'gnomADe_AFR_AF', 'gnomADe_EAS_AF', 'gnomADe_NFE_AF',
                     'gnomADe_AMR_AF', 'gnomADe_SAS_AF', 'gnomADg_AFR_AF',
                     'gnomADg_EAS_AF', 'gnomADg_NFE_AF', 'gnomADg_AMR_AF', 'gnomADg_SAS_AF',
                     'SpliceAI_cutoff', 'phyloP100way_vertebrate_rankscore',
                     'phastCons100way_vertebrate_rankscore']

# Drop the columns
data_init_red = data_init.drop(columns=columns_to_remove)

# Create new_identifier in the dataframe data_init_red
new_id_column = data_init_red.apply(lambda row: '_'.join([str(row['CHROM']), str(row['POS']), row['REF'], row['ALT']]), axis=1)
data_init_red = data_init_red.assign(new_identifier = new_id_column)

# Subset classification_df_final to columns of interest
classification_df_final_red = classification_df_final[['new_identifier', 'Points', 'Classification', 'Criteria']]

# Merge the two dataframes based on the column "new_identifier"
merged_df = pd.merge(data_init_red, classification_df_final_red, on='new_identifier', how='inner') 

# Save to file
merged_df_dir = os.path.join(output_dir, str(sample_name) + "_OncoVI_prediction.xlsx")
merged_df.to_excel(merged_df_dir, index = False)
