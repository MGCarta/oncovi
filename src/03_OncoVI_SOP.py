...
input_data_dir = "V:/gruppen/AG_Bioinfo/members/Carta/PhD_Project/Guidelines_implementation/Preparation_for_submission/05_results/SOP"
...
## Resources 
resources_dir = "V:/gruppen/AG_Bioinfo/members/Carta/PhD_Project/Guidelines_implementation/Preparation_for_submission/03_resources"
...
for filename in os.listdir(input_data_dir):
    ...
    if os.path.isfile(file) and filename.endswith("_prediction.xlsx"):
        ...
        # Read in data    
        data_init = pd.read_excel(file, na_filter=False)  # blank fields are not
        # encoded as nan strings
        ...
        for index, row in data_init.iterrows():
            ...
            # %3D is the way with which VEP annotation indicates variants 
            ...
            # -------------------------------------------------------------------------- ##    
                ...
                # -------------------------------------------------------------------------- ##
                    ...
                    # because an X is reported (e.g. M/X), which does not
                    # codify anything.
                    try:
                        for key in amino_dict.keys():
                            if key in prot_change:
            ...
            # List of AF in gnomAD exome pop. in any 5 general continental
            # pop. . 
            ...
            # List of AF in gnomAD genome pop in any 5 general continental
            # pop. . 
            ...
            ## Criteria: 
            ## Well-established in vitro or in vivio functional studies, supportive of
                    ...
                    if gene in in_frame_indel_dict.keys():
                        ...
                        for key in gene_records_inframe_indel_dict.keys():
                            ...
                            if not '-' in key:
                                ...
                                #amino_acid_pos_joined = '-'.join(amino_acid_pos_list)
            ...
            # Here, the limitation if OS1 is triggered then OM3 is not applicable was 
            # removed. Because the OM3 rule needs OS1 applicable in order to be evaluated.
                ...
                if "missense_variant" in consequence:
                    ...
                    if res == []:
                        ...
                        for item in res:
                            ...
                            # distance not existing between Ile and Ser
                            try:
            ...
            ## Example: retinoblastoma is caused by bi-allelic RB1 inactivation.
        ...
        end_time = time.time()
        print("Elapsed time for the sample in seconds was: ", end_time - start_time)
